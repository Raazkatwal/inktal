...
class CandidateController extends Controller
{
    ...
    public function cv_template(){
        try {
            $data = (new DashboardService())->execute();
            return view('frontend.pages.candidate.cv_template',$data);
        } catch (\Exception $e) {
            flashError('An error occurred: '.$e->getMessage());
    ...
    public function resume1(){
        try {
            $auth = auth()->user();
            $candidate_info = Candidate::with(['languages','experiences','educations','socialInfo','profession_translation','education','experience','skills'])->where('user_id',$auth->id)->first();
            

            return view('frontend.pages.cvs.resume1',compact('auth','candidate_info'));
        } catch (\Exception $e) {
            flashError('An error occurred: '.$e->getMessage());
